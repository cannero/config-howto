...
All elisp is in code blocks and exported to one file with org-babel-tangle =C-c C-v t=.
...
- open: utf-8-auto, it seems to work correctly with patch files, but not mini-buffer input?
...
#+begin_src emacs-lisp :tangle yes :comments org
  ...
  (global-set-key (kbd "C-x <f4>") 'find-file-gdrive-dir)
  (global-set-key (kbd "<f6>") 'my-load-theme)
...
#+begin_src emacs-lisp :tangle yes :comments org
  ;;-------------------------------------------
  ;; set default height
  (cond ((<= 1200 (display-pixel-height))
...
#+begin_src emacs-lisp :tangle yes :comments org
  ...
  ;; set height from https://github.com/seagle0128/doom-modeline/issues/187
  ;; but inactive fontheight is overwritten by themes
  ;; the height is now just set to 0.8 via customize and the font-height function is customized
  (setq doom-modeline-height 1) ; optional
  ;; (if (facep 'mode-line-active)
  ;;     (set-face-attribute 'mode-line-active nil :family "JetBrains Mono" :height 80) ; For 29+
  ;;   (set-face-attribute 'mode-line nil :family "JetBrains Mono" :height 80))
  ;; (set-face-attribute 'mode-line-inactive nil :family "JetBrains Mono" :height 80)

  (defun my-doom-modeline--font-height ()
    "Calculate the actual char height of the mode-line."
    (* (+ (frame-char-height) 2) 0.8))
...
#+begin_src emacs-lisp :tangle yes :comments org
  (use-package dired
    ...
    (setq dired-listing-switches
          "-aGFhl --group-directories-first --time-style=long-iso"))
#+end_src
*** which-function
#+begin_src emacs-lisp :tangle yes :comments org
  (eval-after-load "which-func"
    '(setq which-func-modes '(python-mode python-ts-mode csharp-ts-mode powershell-mode powershell-ts-mode)))
#+end_src
*** whitespace-mode
#+begin_src emacs-lisp :tangle yes :comments org
...
#+begin_src emacs-lisp :tangle yes :comments org
  (dir-locals-set-class-variables 'servo-directory
                                  '((rustic-mode . ((lsp-rust-analyzer-cargo-override-command . [".\\mach.bat" "check" "--message-format=json" "--target-dir=target/lsp"])
...
#+begin_src emacs-lisp :tangle yes :comments org
  (defun my-python-mode-hook ()
    ...
    ;; venv support, activate with dir/venv
    (pyvenv-mode)

    ;; tried also anaconda-mode, conda was working out of the box
...
#+begin_src emacs-lisp :tangle yes :comments org
  (defun my-csharp-mode-fn ()
    ...
    (setq-default c-basic-offset 4
                  ...
                  csharp-ts-mode-indent-offset 4)
    ...
    (add-hook  'csharp-ts-mode-hook 'my-csharp-mode-fn t)
...
#+begin_src emacs-lisp :tangle yes :comments org
  ...
  (add-to-list 'auto-mode-alist '("\\.[cm]?js[mx]?\\'" . js-mode))
...
#+begin_src emacs-lisp :tangle yes :comments org
  ...
  (setq howm-prefix (kbd "\C-c o"))
  (setq howm-file-name-format "%Y/%m/%Y-%m-%d-%H%M%S.org")
  (setq howm-view-title-header "*")
  (require 'howm)
  ;(setq howm-directory (file-name-concat (getenv "TODO set path here") "howm"))
#+end_src
*** c++ mode
#+begin_src emacs-lisp :tangle yes :comments org
...
#+begin_src emacs-lisp :tangle yes :comments org
  (defun my-zig-mode-hook ()
    (company-mode)
    (setq zig-zig-bin "C:/tools/zig/zig.exe")
    (require 'lsp-mode)
    (setq lsp-zig-zls-executable "C:/tools/zig/zls/zls.exe")
    (setq lsp-zig-zig-exe-path "C:/tools/zig/zig.exe")
    (lsp-deferred))
...
#+begin_src emacs-lisp :tangle yes :comments org
  ...
  (add-hook 'swift-ts-mode-hook 'my-swift-mode-hook)
#+end_src

*** powershell-ts mode
#+begin_src emacs-lisp :tangle yes :comments org
  ; https://github.com/dmille56/powershell-ts-mode/blob/main/powershell-ts-mode.el
  ...
  (defun my-powershell-mode-hook ()
    ...
    (setq mode-line-misc-info
          (assq-delete-all 'which-function-mode mode-line-misc-info)))
  (add-hook 'powershell-ts-mode-hook 'my-powershell-mode-hook)
#+end_src
...
#+begin_src emacs-lisp :tangle yes :comments org
  ...
  ;; remap tree sitter modes, see https://www.masteringemacs.org/article/how-to-get-started-tree-sitter
...
#+begin_src emacs-lisp :tangle yes :comments org
  ...
  ;; As long as only hunspell is in the path, (setq ispell-program-name "hunspell") must not be used
  ;; From https://www.reddit.com/r/emacs/comments/dgj0ae/tutorial_spellchecking_with_hunspell_170_for/?rdt=37030
  (setq ispell-local-dictionary "en_US")
  (setq ispell-hunspell-dict-paths-alist
        '(("en_US" "C:/TODO_SET_PATH/en_US.aff")))
  (setq ispell-local-dictionary-alist
        ;; Please note the list `("-d" "en_US")` contains ACTUAL parameters passed to hunspell
        ;; You could use `("-d" "en_US,en_US-med")` to check with multiple dictionaries
        '(("en_US" "[[:alpha:]]" "[^[:alpha:]]" "[']" nil ("-d" "en_US") nil utf-8)))
  ;; env needs to be set additionally
  (setenv "LANG" "en_US")
...
#+begin_src emacs-lisp :tangle yes :comments org
  ;; authinfo entry
  ;; machine generativelanguage.googleapis.com login apikey password the_api_key
  (use-package gptel
    :ensure t
    :config
    (setq
...
#+begin_src emacs-lisp :tangle yes :comments org
  ...
  ;; to get path/filename use C-x C-j C-0 w
...
#+begin_src emacs-lisp :tangle yes :comments org
  ...
  (custom-set-faces
   ...
   ;; Your init file should contain only one such instance.
   ;; If there is more than one, they won't work right.
   '(default ((t (:inherit nil :extend nil :stipple nil :inverse-video nil :box nil :strike-through nil :overline nil :underline nil :slant normal :weight regular :height 128 :width normal :foundry "outline" :family "Fantasque Sans Mono"))))
   '(header-line ((t (:inherit mode-line :height 0.9))))
   '(mode-line ((t (:box nil))))
   '(mode-line-active ((t (:inherit mode-line :height 0.8))))
   '(mode-line-inactive ((t (:box nil :height 0.8)))))
#+end_src
