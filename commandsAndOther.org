...
** WSL
access path with \\wsl$, emacs shows it as //wsl$$/Ubuntu/home/<user>/
from app also 'explorer.exe .' is possible as non-root user
restart with Get-Service LxssManager | Restart-Service
*** ubuntu
...
** packages
put packages in folder ~/vimfiles/pack/bundle/start/
* git
** basics
