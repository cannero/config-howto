...
**** select into
SELECT * FROM t1 INTO @myvar
**** kill all processes

#+begin_src powershell-ts
  $conns = $(mysql -u dbuser -ppwd -P 3307 -h server --disable-column-names -e "SELECT CONCAT('KILL ', p.ID, ';') FROM information_schema.processlist p WHERE p.`HOST` LIKE '10.150.0.156:%' AND p.`USER` = 'user' AND p.`DB` = 'db';")
  mysql -u dbuser -ppwd -P 3307 -h server --disable-column-names -e "$conns"
#+end_src
* Debugging
...
** IIS
   ...
   - users
     ...
     set location to PC when searching using 'check name'
...
- string to number
  (string-to-number "100" 2)
  if conversion could fail: cl-parse-integer
- convert to string rep
  (require 'calc-bin)
  (let ((calc-number-radix 16))
   (math-format-radix 150))
...
- cURL
  ...
  -O: output to file
  - send base64 encoded string from image
    curl -X POST -H "Content-Type: application/json" -d '{ "imageData64": "'$(base64 /c/TEMP/small.jpg | tr -d '[:space:]')'", "filename": "image.jpg" }' http://localhost:65100/UploadImage
- wget
...
- git blame --reverse START.. file.ext
  gives last commit where line was present
  --first-parent if multiple merges
  then search for next commit
  git log --reverse --ancestry-path COMMIT^..main
...
stores pwd/token after the next commit in ~/.git-credentials
re-fresh password by running the failing cmd a second time
* jj
** basic workflow with git backend
*** init
- jj git init
- jj git remote add origin https://github.com/cannero/rlox.git
...
- jj bookmark set main
  ...
  jj bookmark move --from main --to @- --allow-backwards
- jj git push
** commands
- jj show
  show changes
...
- jj split -i
  select changes which stay
- jj squash
  everything, file  or -i
  --into <rev>
- jj log --limit 5
- jj next --edit
- jj edit <rev>
  or use new <rev> and squash
...
** commands
   ...
   - redirect Write-Host to file
     6>file_name
   - run exe on each file
     Get-ChildItem ... | ForEach-Object { &exe $_.FullName}
*** format files output
...
* Krita
** font size
set resolution to a lower value, e.g. 100ppi
Image > Scale Image
* Notepad++
- delete %AppData%\Notepad++\session.xml to remove session
